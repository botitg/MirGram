...
app.post("/api/auth/register", withApi(async (req, res) => {
...
app.post("/api/auth/login", withApi(async (req, res) => {
    const username = normalizeUsername(req.body.username);
    const password = String(req.body.password || "");
...
app.get("/api/auth/me", authMiddleware, withApi(async (req, res) => {
    const user = await readUser(req.user.id);
    res.json({ user });
}));

app.put("/api/profile", authMiddleware, withApi(async (req, res) => {
...
app.get("/api/users/search", authMiddleware, withApi(async (req, res) => {
...
app.get("/api/chats/:chatId", authMiddleware, withApi(async (req, res) => {
...
app.post("/api/chats/private", authMiddleware, withApi(async (req, res) => {
...
app.post("/api/chats/group", authMiddleware, withApi(async (req, res) => {
...
app.post("/api/chats/:chatId/members", authMiddleware, withApi(async (req, res) => {
    const chatId = Number(req.params.chatId);
...
app.put("/api/chats/:chatId/members/:memberId", authMiddleware, withApi(async (req, res) => {
    const chatId = Number(req.params.chatId);
    ...
    await db.run(
        ...
        [nextRole, groupNick || null, groupAvatarUrl || null, canSend ? 1 : 0, canSendMedia ? 1 : 0, canStartCalls ? 1 : 0, chatId, targetId],
    );
...
app.put("/api/chats/:chatId/me", authMiddleware, withApi(async (req, res) => {
    const chatId = Number(req.params.chatId);
    const membership = await readMembership(chatId, req.user.id);
...
app.get("/api/chats/:chatId/messages", authMiddleware, withApi(async (req, res) => {
    const chatId = Number(req.params.chatId);
...
app.post("/api/chats/:chatId/messages", authMiddleware, withApi(async (req, res) => {
    const chatId = Number(req.params.chatId);
...
app.post("/api/chats/:chatId/messages/image", authMiddleware, (req, res, next) => {
...
async function getCallParticipantsPayload(chatId) {
    ...
    const rows = await db.all(
        `SELECT id, username, avatar_url AS avatarUrl
         FROM users
         WHERE id IN (${Array.from(call.participants).map(() => "?").join(",")})`,
        [...call.participants],
    );
